...
import { Car, Wrench, CheckCircle, Plus, X, Gauge, Search } from 'lucide-react';
...
const Vehicles: React.FC = () => {
    ...
    const FilterButton: React.FC<{ status: 'all' | Vehicle['status']; label: string }> = ({ status, label }) => (
         <button
            onClick={() => setStatusFilter(status)}
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
                statusFilter === status ? 'bg-primary text-white' : 'bg-white text-gray-700 hover:bg-gray-100'
            }`}
        >
            {label}
        </button>
    ...
    return (
        <div>
            ...
            {/* Filters */}
            <div className="mb-6 p-2 bg-gray-50 rounded-lg flex items-center justify-start">
                <div className="flex items-center space-x-2 p-1 bg-gray-200 rounded-lg">
                   <FilterButton status="all" label="Vše" />
                   <FilterButton status="available" label="K dispozici" />
                   <FilterButton status="rented" label="Pronajato" />
                   <FilterButton status="maintenance" label="V servisu" />
                </div>
            </div>
            ...
            {filteredVehicles.length > 0 ? (
                ...
                <div className="text-center py-16 bg-white rounded-lg shadow-md">
                    ...
                    <p className="text-gray-500 mt-2">Zkuste upravit filtry nebo zkontrolovat zadaný text.</p>
                </div>
            )}
        </div>
    );
